...
export async function runAdsAgent({ task, campaignId, campaignName, channelId, slackClient, dependencyContext = [] }) {
  ...
  // Primary output is the main headline + primary_text
  ...
  const msg = await slackClient.chat.postMessage({
    channel: channelId,
    text:    `Ad copy ready for approval: ${task.type}`,
...
function buildPrompt({ product, platform, description, params, memory, dependencyContext }) {
  const parts = [
    `Product: ${product}`,
    `Ad Platform: ${platform ?? task?.type ?? 'meta'}`,
    `Task: ${description}`,
  ]
